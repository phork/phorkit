...
import { SizeConsumer, SizeProvider, useGetSize, useObserveSize, useSizeListeners } from '../index';
                  ...
                  <Rhythm my={2}>
                    <div>
                      {`The ${observe ? 'current' : 'last observed'} width of this element is ${
                        width === undefined ? 'undefined' : `${Math.round(width)}px`
                      }`}
                    </div>
                    <div>
                      {`The ${observe ? 'current' : 'last observed'} height of this element is ${
                        height === undefined ? 'undefined' : `${Math.round(height)}px`
                      }`}
                    </div>
                  </Rhythm>
                </div>
    ...
    {({ state: props, setState: setProps }) => {
      return (
        <React.Fragment>
          ...
          <SizeProvider observe propsToMeasure={props}>
            {ref => (
              <SizeConsumer>
                {({ width, height, top, bottom, left, right }) => (
                  <div ref={ref} style={{ background: '#556270', color: '#fff', padding: 16, width: '100%' }}>
                    <Rhythm my={2}>
                      {props.includes('width') && <div>{`The width of this element is ${Math.round(width)}px`}</div>}
                      {props.includes('height') && <div>{`The height of this element is ${Math.round(height)}px`}</div>}
                      {props.includes('top') && <div>{`The top of this element is ${Math.round(top)}px`}</div>}
                      {props.includes('bottom') && <div>{`The bottom of this element is ${Math.round(bottom)}px`}</div>}
                      {props.includes('left') && <div>{`The left of this element is ${Math.round(left)}px`}</div>}
                      {props.includes('right') && <div>{`The right of this element is ${Math.round(right)}px`}</div>}
                    </Rhythm>
                  </div>
                )}
              </SizeConsumer>
              ...
              <SizeConsumer>
                {({ width, height }) => (
                  <div style={{ width: '100%', height: '210px' }}>
                    <div ref={ref} style={{ background: '#556270', color: '#fff', padding: 16, width: '100%' }}>
                      <Rhythm my={2}>
                        <div>{`The width of this element is ${Math.round(width)}px`}</div>
                        <div>{`The height of this element is ${Math.round(height)}px`}</div>
                      </Rhythm>
...
<Props of={useObserveSize} />

### useSizeListeners

<Props of={useSizeListeners} />
