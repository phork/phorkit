---
name: All components
menu: Forms
route: /components/FormAll
---

import { Playground, Props } from 'docz';
import { SearchIcon } from 'icons/SearchIcon';
import { Button, ButtonGroup } from 'components/Button';
import { Flex } from 'components/Flex';
import { Rhythm } from 'components/Rhythm';
import { ControlledDropdown } from 'compositions/Dropdown';
import { StateWrapper } from 'docs/helpers/StateWrapper';
import { ThemeWrapper } from 'docs/helpers/ThemeWrapper';
import {
  Checkbox,
  CheckboxGroup,
  Label,
  Password,
  Radio,
  RadioGroup,
  Select,
  Slider,
  Stepper,
  Textarea,
  Textbox,
  Toggle,
} from '../index';
import { FormDemo } from './helpers/FormDemo';

# All form components

<Playground>
  <StateWrapper initialState={{ variant: 'underline', size: 'large' }}>
    {({ state, setState }) => (
      <ThemeWrapper>
        <Rhythm mx={3} grouped>
          <FormDemo contrast={false} style={{ maxWidth: '400px' }}>
            {({
              contrast = false,
              handleChange: onChange,
              handleClear: onClear,
              options,
              persistEvents,
              refs,
              setValues,
              values,
            }) => (
              <React.Fragment>
                <Rhythm mt={4} mb={2}>
                  <ButtonGroup
                    buttons={[
                      { id: 'underline', label: 'Underline', selected: state.variant === 'underline' },
                      { id: 'filled', label: 'Filled', selected: state.variant === 'filled' },
                      { id: 'outline', label: 'Outline', selected: state.variant === 'outline' },
                    ]}
                    color="primary"
                    fullWidth
                    onClick={(event, id) => setState({ ...state, variant: id })}
                    shape="brick"
                    spacing="joined"
                  />
                </Rhythm>
                <Rhythm mt={4} mb={2}>
                  <ButtonGroup
                    buttons={[
                      { id: 'medium', label: 'M', selected: state.size === 'medium' },
                      { id: 'large', label: 'L', selected: state.size === 'large' },
                      { id: 'xlarge', label: 'XL', selected: state.size === 'xlarge' },
                      { id: 'xxlarge', label: '2XL', selected: state.size === 'xxlarge' },
                      { id: 'xxxlarge', label: '3XL', selected: state.size === 'xxxlarge' },
                      { id: 'xxxxlarge', label: '4XL', selected: state.size === 'xxxxlarge' },
                      { id: 'xxxxxlarge', label: '5XL', selected: state.size === 'xxxxxlarge' },
                      { id: 'xxxxxxlarge', label: '6XL', selected: state.size === 'xxxxxxlarge' },
                      { id: 'xxxxxxxlarge', label: '7XL', selected: state.size === 'xxxxxxxlarge' },
                    ]}
                    color="neutral"
                    fullWidth
                    onClick={(event, id) => setState({ ...state, size: id })}
                    selectedColor="primary"
                    selectedWeight="inline"
                    shape="brick"
                    size="small"
                    spacing="cozy"
                    weight="inline"
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Textbox
                    contrast={contrast}
                    label="Username"
                    name="username"
                    onChange={onChange}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.username = e;
                    }}
                    size={state.size}
                    transitional
                    value={values.username}
                    variant={state.variant}
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Password
                    contrast={contrast}
                    label="Password"
                    name="password"
                    onChange={onChange}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.password = e;
                    }}
                    size={state.size}
                    transitional
                    value={values.password}
                    variant={state.variant}
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Textbox
                    clearable
                    contrast={contrast}
                    iconBefore={<SearchIcon />}
                    label="Search"
                    name="search"
                    onChange={(event, value) => setValues({ ...values, search: value })}
                    onClear={onClear}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.search = e;
                    }}
                    size={state.size}
                    transitional
                    value={values.search}
                    variant={state.variant}
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Textbox
                    contrast={contrast}
                    label="Successful"
                    name="success"
                    onChange={onChange}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.success = e;
                    }}
                    size={state.size}
                    transitional
                    validity="success"
                    value={values.success}
                    variant={state.variant}
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Textbox
                    contrast={contrast}
                    label="Warningish"
                    name="warning"
                    onChange={onChange}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.warning = e;
                    }}
                    size={state.size}
                    transitional
                    validity="warning"
                    value={values.warning}
                    variant={state.variant}
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Textbox
                    contrast={contrast}
                    label="Dangerous"
                    name="danger"
                    onChange={onChange}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.danger = e;
                    }}
                    size={state.size}
                    transitional
                    validity="danger"
                    value={values.danger}
                    variant={state.variant}
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Select
                    contrast={contrast}
                    label="Choose a color"
                    name="selectColor"
                    onChange={onChange}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.selectColor = e;
                    }}
                    size={state.size}
                    transitional
                    value={values.selectColor}
                    options={options}
                    variant={state.variant}
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Select
                    contrast={contrast}
                    label="Choose another color"
                    name="selectAnotherColor"
                    onChange={onChange}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.selectAnotherColor = e;
                    }}
                    size={state.size}
                    transitional
                    value={values.selectAnotherColor}
                    options={options}
                    variant={state.variant}
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <ControlledDropdown
                    inputVariant={state.variant}
                    label="A custom dropdown"
                    layout="raised"
                    onChange={value => onChange({ target: { name: 'dropdown' } }, value)}
                    options={options}
                    ref={e => {
                      refs.current.dropdown = e && e.container;
                    }}
                    size={state.size}
                    transitional
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <ControlledDropdown
                    inputVariant={state.variant}
                    label="A contained dropdown"
                    layout="contained"
                    onChange={value => onChange({ target: { name: 'dropdownContained' } }, value)}
                    options={options}
                    ref={e => {
                      refs.current.dropdownContained = e && e.container;
                    }}
                    size={state.size}
                    transitional
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Textarea
                    contrast={contrast}
                    label="Manifesto"
                    name="textarea"
                    onChange={onChange}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.textarea = e;
                    }}
                    size={state.size}
                    transitional
                    value={values.textarea}
                    variant={state.variant}
                  />
                </Rhythm>
                <Rhythm my={2}>
                  <Checkbox
                    checked={!!values.checkbox}
                    contrast={contrast}
                    name="checkbox"
                    onChange={(event, checked) => setValues({ ...values, checkbox: checked })}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.checkbox = e;
                    }}
                    variant="secondary"
                  >
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer eget dui eu orci ultrices maximus
                    eu eu ante. Vestibulum ligula velit, scelerisque vitae venenatis tempor, consectetur a orci. Aenean
                    molestie eu tortor in pharetra. Nulla lobortis, sapien quis ornare venenatis, nunc libero finibus
                    justo, non tempor nisi urna quis justo. Integer sit amet venenatis leo.
                  </Checkbox>
                </Rhythm>
                <Rhythm my={2} mr={2}>
                  <Radio
                    checked={values.radio === 'one'}
                    contrast={contrast}
                    name="radio"
                    value="one"
                    onChange={(event, checked, value) => checked && setValues({ ...values, radio: value })}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.radio1 = e;
                    }}
                    variant="secondary"
                  >
                    Lorem ipsum dolor sit amet
                  </Radio>
                  <Radio
                    checked={values.radio === 'two'}
                    contrast={contrast}
                    name="radio"
                    value="two"
                    onChange={(event, checked, value) => checked && setValues({ ...values, radio: value })}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.radio2 = e;
                    }}
                    variant="secondary"
                  >
                    Consectetur adipiscing elit
                  </Radio>
                </Rhythm>
                <Rhythm mt={4} mb={2} grouped>
                  <Flex direction="row" alignItems="flex-end" justifyContent="space-between" flexible>
                    <div>
                      <Label as="legend" contrast={contrast} strength="legend">
                        Pick a big number
                      </Label>
                      <Stepper
                        contrast={contrast}
                        inputWidth={100}
                        max={10000}
                        min={0}
                        name="stepper1"
                        onChange={(event, value) => setValues({ ...values, stepper1: value })}
                        persistEvents={persistEvents}
                        ref={e => {
                          refs.current.stepper1 = e;
                        }}
                        size={state.size}
                        step={100}
                        value={values.stepper1}
                        width="50%"
                        variant={state.variant}
                      />
                    </div>
                    <Stepper
                      contrast={contrast}
                      inputWidth={100}
                      label="Pick another number"
                      max={100}
                      min={0}
                      size={state.size}
                      step={10}
                      name="stepper2"
                      onChange={(event, value) => setValues({ ...values, stepper2: value })}
                      persistEvents={persistEvents}
                      ref={e => {
                        refs.current.stepper2 = e;
                      }}
                      value={values.stepper2}
                      width="50%"
                      variant={state.variant}
                    />
                  </Flex>
                </Rhythm>
                <Rhythm mt={4} mb={2}>
                  <CheckboxGroup
                    contrast={contrast}
                    layout="stacked"
                    legend="Super fantastic label says what?"
                    name="checkboxes"
                    onChange={(event, value) => setValues({ ...values, checkboxes: value })}
                    values={values.checkboxes}
                    checkboxes={[
                      {
                        id: 'demo-checkbox-group1',
                        label: 'Super fantastic label one',
                        name: 'checkboxes1',
                        ref: refs.checkboxes1,
                        value: 'one',
                      },
                      {
                        id: 'demo-checkbox-group2',
                        label: 'Super fantastic label two',
                        name: 'checkboxes2',
                        ref: refs.checkboxes2,
                        value: 'two',
                      },
                      {
                        id: 'demo-checkbox-group3',
                        label: 'Super fantastic label three',
                        name: 'checkboxes3',
                        ref: refs.checkboxes3,
                        value: 'three',
                      },
                    ]}
                  />
                </Rhythm>
                <Rhythm mt={4} mb={2}>
                  <RadioGroup
                    contrast={contrast}
                    layout="inline"
                    legend="Super fantastic label says what?"
                    name="radios"
                    onChange={(event, value) => setValues({ ...values, radios: value })}
                    value={values.radios}
                    radios={[
                      {
                        id: 'demo-radio-group1',
                        label: 'Super fantastic label one',
                        ref: refs.radios1,
                        value: 'one',
                      },
                      {
                        id: 'demo-radio-group2',
                        label: 'Super fantastic label two',
                        ref: refs.radios2,
                        value: 'two',
                      },
                      {
                        id: 'demo-radio-group3',
                        label: 'Super fantastic label three',
                        ref: refs.radios3,
                        value: 'three',
                      },
                    ]}
                  />
                </Rhythm>
                <Rhythm mt={4} mb={2}>
                  <Slider
                    contrast={contrast}
                    formatValue={value => value + '%'}
                    max={100}
                    min={0}
                    name="slider"
                    onChange={(event, value) => setValues({ ...values, slider: value })}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.slider = e;
                    }}
                    value={values.slider}
                    valuePosition="top"
                    width="100%"
                  >
                    Percentage
                  </Slider>
                </Rhythm>
                <Flex justifyContent="space-between" alignItems="center" wrap>
                  <Toggle
                    checked={values.toggle}
                    contrast={contrast}
                    name="toggle"
                    onChange={(event, checked) => setValues({ ...values, toggle: checked })}
                    persistEvents={persistEvents}
                    ref={e => {
                      refs.current.toggle = e;
                    }}
                    value={true}
                  >
                    Activate super powers
                  </Toggle>
                  <Rhythm mr={-2}>
                    <Flex flexible alignItems="center" justifyContent="flex-end">
                      <Rhythm my={1} mr={2}>
                        <Button weight="ghost" color="neutral" shape="brick">
                          Cancel
                        </Button>
                        <Button type="submit" color="primary" shape="brick" onClick={() => console.log(refs.current)}>
                          Submit
                        </Button>
                      </Rhythm>
                    </Flex>
                  </Rhythm>
                </Flex>
              </React.Fragment>
            )}
          </FormDemo>
        </Rhythm>
      </ThemeWrapper>
    )}
  </StateWrapper>
</Playground>
