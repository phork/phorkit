---
name: Divider
menu: Utilities
route: /components/Divider
---

import { Playground, Props } from 'docz';
import { Divider, ColoredDivider } from '../index';
import { Looper } from 'docs/helpers/Looper';
import { ColorLooper } from 'docs/helpers/ColorLooper';
import { ThemeWrapper } from 'docs/helpers/ThemeWrapper';
import { PageTitle } from 'docs/helpers/PageTitle';

<PageTitle title="Divider" src="components/Divider" />

## Horizontal divider

<Playground>
  <ThemeWrapper>
    <Divider />
  </ThemeWrapper>
</Playground>

## Vertical divider

<Playground>
  <ThemeWrapper>
    <div style={{height: '60px'}}>
      <Divider orientation="vertical" />
    </div>
  </ThemeWrapper>
</Playground>

## Variations

### Basic divider

<Playground>
  <ThemeWrapper>
    <Looper list={[undefined, 'quiet', 'quieter', 'quietest']} render={volume => (
      <div key={volume || 'none'} style={{marginBottom: '12px'}}>
        <h4>volume: {volume || 'none'}</h4>
        <Divider variant="primary" volume={volume} />
      </div>
    )} />
  </ThemeWrapper>
</Playground>

### Contrast divider

<Playground>
  <ThemeWrapper contrast>
    <Looper list={[undefined, 'quiet', 'quieter', 'quietest']} render={volume => (
      <div key={volume || 'none'} style={{marginBottom: '12px'}}>
        <h4>volume: {volume || 'none'}</h4>
        <Divider contrast volume={volume} />
      </div>
    )} />
  </ThemeWrapper>
</Playground>

## Colored divider

<Playground>
  <ThemeWrapper withThemeId>
    {({ themeId }) => (
      <React.Fragment>
        <ColorLooper themeId={themeId} group="primary" render={({ id }) => (
          <div key={id} style={{marginBottom: '12px'}}>
            <h4>Color: {id}</h4>
            <ColoredDivider key={id} themeId={themeId} colorId={id} />
          </div>
        )} />
        <ColorLooper themeId={themeId} group="neutral" render={({ id }) => (
          <div key={id} style={{marginBottom: '12px'}}>
            <h4>Color: {id}</h4>
            <ColoredDivider key={id} themeId={themeId} colorId={id} />
          </div>
        )} />
      </React.Fragment>
    )}
  </ThemeWrapper>
</Playground>

## \[props\]

### Divider

<Props of={Divider} />

### ColoredDivider

<Props of={ColoredDivider} />
