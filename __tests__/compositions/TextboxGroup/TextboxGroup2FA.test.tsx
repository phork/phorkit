...
describe('<TextboxGroup2FA />', () => {
  ...
  it('should trigger the change event', async () => {
    ...
    await userEvent.type(textboxes[0], '4');
    ...
    await userEvent.type(textboxes[1], '3');
  ...
  it('should use a space for a missing input value', async () => {
    const onChange = jest.fn();

    const { container } = render(<TextboxGroup2FA length={6} onChange={onChange} value="432" />);
    const textboxes = container.querySelectorAll('input');

    await userEvent.type(textboxes[4], '0');
    expect(onChange).toHaveBeenCalledTimes(1);
